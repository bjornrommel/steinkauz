...
1.1.0   corrected the derivative of the phase velocity
        plus some improvements to the graphics, but the parameter transfer 
        misses respective options
"""
...
from IPython import get_ipython        # ipython for graphics control
...
SEG = True             # follow SEG figure guideline (if VELPLOT or TIMEPLOT)
...
def precompute(
    ...
    if VELPLOT:
        polarplot(vel=energy, seg=seg)   # comment out if not desired
...
def polarplot(vel=None, title=TITLE, seg=SEG):
    """
    ...
    # plot title
    plt.title(title + ": Energy Velocity")
    # SEG style
    if seg:
        setfont()
    # save
...
def traveltime(xxx=None, zzz=None, scs=None, seg=SEG):
    """
...
def travelplot(xxx=None, time=None, title=TITLE, seg=SEG):
    """
