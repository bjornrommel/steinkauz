# -*- coding: utf-8 -*-
r"""
Steinkauz -- write Latex in a Jupyter environment.
...
MAINDELETE = [
    'AUX', 'dvi', 'epi', 'eps', 'log', 'png', 'png64', 'ps', 'pyc', 'synctex',
    'synctex(busy)']
MAINFOLDER = ['.']
...
class Steinkauz(Magics):
    ...
    def __init__(self, shell, project):
        ...
        self.project = cp(project) \
            if project else {}            # externally provided LaTeX counters
...
# -----------------------------------------------------------------------------
    ...
    def proc_args(self, args=None):    # pylint: disable=unused-argument   # ... proc_<xxx>
        """
...
# -----------------------------------------------------------------------------
    ...
    def proc_cmd_setup(self, cell=None):
        ...
        with open(TEMPLATEFILE, 'w', encoding='utf-8') as file:
            file.write(template)
    ...
    def proc_cmd_clean(self, cell=None):   # pylint:disable=unused-argument
        ...
        def cleanfile(file):
            try:
                ...
                string = f"!!! cleanfile: file {file} not found !!!\n"
                print(string)
                if DEBUG:
                    raise UserWarning(msg) from msg
            except PermissionError as msg:
                string = f"!!! cleanfile: file {file} in use !!!\n"
                print(string)
                if DEBUG:
                    raise UserWarning(msg) from msg
...
# -----------------------------------------------------------------------------
    ...
    def make_incnt(self):
        ...
        counterfile = self.fullname(
            ...
            file='INCOUNTER')
        # write to file
        with open(counterfile, 'w', encoding='utf-8') as file:
    ...
    def get_tpl(self):
        ...
        # read the tpl file
        try:
            with open(tplfile, 'r', encoding='utf-8') as file:
                self.fragment = file.read()
        except FileNotFoundError as msg:
            string = "!!! get_tpl: tplfile cannot be found !!!\n"
            string += f"texdir: {self.texdir}\n"
            string += f"texfile: {self.texfile}\n"
            print(string)
            raise UserWarning(msg) from msg
    ...
    def put_tpl(self):
        ...
        # write the tpl file
        try:
            with open(tplfile, 'w', encoding='utf-8') as file:
                out = self.fragment
                if out[0:4] != 'r"""':
                    out = 'r"""\n' + out + '\n"""'
                file.write(out)
...
# -----------------------------------------------------------------------------
    ...
    def run_cmd(   # pylint:disable=too-many-arguments
        ...
        def message(cmd=None, shell=None, cwd=None):
            ...
            string = f"command: {cmd}\n"
            string = f"shell: {shell}\n"
            string = f"cwd: {cwd}\n"
            return string
        ...
        if proc.returncode > 0:
            ...
            string += f"stdout: {proc.stdout}"
            string += f"stderr: {proc.stderr}"
            raise UserWarning(string)
...
def load_ipython_extension(ipython):
    ...
    if not os.path.isdir(STEINKAUZDIR):
        try:
            ...
            string += f"STEINKAUZDIR folder {STEINKAUZDIR}\n"
            print(string)
            raise UserWarning(msg) from msg
    # define version
    if not os.path.isfile('VERSION'):
        try:
            with open(VERSIONFILE, 'w', encoding='utf-8') as file:
    ...
    try:
        ...
        string += f"TEMPLATEFILE file {TEMPLATEFILE}\n"
        print(string)
        raise UserWarning(msg) from msg
